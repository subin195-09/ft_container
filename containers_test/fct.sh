...
compile () {
	...
	$CC $CFLAGS -o "$3" -I./$incl_path$5 -DTESTED_NAMESPACE=$2 $1 &>$4
	return $?
}
...
cmp_one () {
	...
	compile "$1" "ft"  "$ft_bin" /dev/null $2;  ft_ret=$?
	compile "$1" "std" "$std_bin" $std_compile_log $2; std_ret=$?
